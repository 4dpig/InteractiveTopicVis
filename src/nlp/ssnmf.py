...
class SSNMFTopicModel:
    def __init__(self,
        ...
        self.random_init_wh()
        # dh初始化为单位矩阵
        self.dh = np.identity(self.n, dtype=np.float32)
        ...
        self.mh = np.zeros((self.n, self.n), dtype=np.float32)
    ...
    def error_semi_supervised_nmf(self):
        ...
        计算半监督nmf的损失函数值
        """
        matrix1 = self.a - np.dot(self.w, self.h)
        error1 = squared_norm(matrix1)

        matrix2 = np.dot(self.w - self.wr, self.mw)
        error2 = squared_norm(matrix2)

        matrix3 = np.dot(self.h - np.dot(self.hr, self.dh), self.mh)
        ...
        for i in range(self.max_iter):
            if i % 2 == 0:
                ...
                for ci in range(self.n):
                    ...
                    vstack_aci_mhi_dhi_hrci = np.vstack(
                        (
                            self.a[:, ci].reshape((self.m, 1)),
                            (self.mh[ci, ci] * self.dh[ci, ci] * self.hr[:, ci]).reshape(
                                (self.k, 1))
                        )
                    )
